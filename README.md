...
**Цель:** Разработка рекомендаций по сбору и хранению данных, их оптимизация. Дать ответы на вопросы об изменении числа ДТП по 
временным промежуткам и разлчичие числа ДТП для водителей с разным стажем. 
...
## Анализ данных для команды игры "Секреты Темнолесья"
**Описание:** Заказчик - команда кмпьютерной игры «Секреты Темнолесья». Команда игры хочет привлечь новую аудиторию и подготовить статью о развитии индустрии игр в начале XXI века. Для статьи понадобится изучить развитие игровой индустрии с 2000 по 2013 год. 

**Цель:** Проверить корректность данных и провести предобработку, получив необходимый срез данных. 

**Итоги:**
- Проведена предобработку данных, были убраны дубликаты и пропуски, проерена корректность числовых и временных значений.
- Отобраны данные по времени выхода игры. Период с 2000 по 2013 год включительно.
- Категоризованы игры по оценкам пользователей и экспертов. Выделены три категории:
...
- Найдены топ-7 платформ по количеству игр, выпущенных за весь требуемый период.
  
[Ссылка на проект](https://github.com/averkinama/yandex_practicum_projects/blob/main/data_analysis_temnolesya_secrets.ipynb)
___

## Анализ метрик приложения "Procrastinate Pro+"
**Описание:**Приложение Procrastinate Pro+ предоставляет пользователям доступ к развлекательному контенту: видео, подкастам и блогам. Пользователи могут выбрать несколько пакетов контента. Пакеты различаются по цене, объёму и содержанию.
К началу лета в приложении появился новый вид контента — спортивный. 
...
**Цель:** Выявить причины стагнации выручки.

**Итоги:**
- Проведен анализ юнит-экономики продукта в динамике за первые 28 дней.
- Рассчитаны такие метрики, как DAU, MAU и Stickiness в общем по данным и в разрезе каналов привлечения, средняя стоимость привлечения пользователя (САС), Значение LTV и ROI с учётом покупок, совершённых за 28 дней с момента привлечения, Retention Rate на 14-й день с момента привлечения (по каналам), Конверсия в покупку (по каналам). Все метрики имеют визуализацию.
- Найдены и обоснованы причины стагнации выручки.
- Было определено, какие источники привлечения приносят прибыль, а какие не выходят на уровень окупаемости.
- Рассчитаны такие метрики, как ROI и Payback Period для источников, которые к 28-му дню не выходят на уровень окупаемости. Все метрики имеют визуализацию.
- Даны рекомендации отделу маркетинга.
  
[Ссылка на проект](https://github.com/averkinama/yandex_practicum_projects/blob/main/procratinatepro_metric_systeam.ipynb)
___
